...
def train_or_eval_model(args, model, reg_loss, cls_loss, dataloader, optimizer=None, train=False, first_stage=True, mark='train',probabilities=[0.5,0.5,0.5]):
    ...
    pro=[0.7,0.1]
    # all_modalities = 3*len(dataloader)
    available_modalities = 3
    num_two_modalities_max = len(dataloader)*pro[1]
    num_one_modalities_max = len(dataloader)*pro[0]
    cur_two_modalities = 0
    cur_one_modalities = 0
    for data_idx, data in enumerate(dataloader):
        ...
        elif cur_one_modalities < num_one_modalities_max:
            available_modalities = 1
            cur_one_modalities += 1
        else:
            available_modalities = 3
        ## using cmp-net masking manner [at least one view exists]
        ## host mask
        # ef generate_mask(seqlen, batch, test_condition, first_stage,train,probabilities):
        ...
        if dataset in ['IEMOCAPFour', 'IEMOCAPSix']:
            if first_stage:
                ...
                # loss_at = cls_loss(lp_a_t, labels_, umask)
                # loss_av = cls_loss(lp_a_v, labels_, umask)
                # loss_ta= cls_loss(lp_t_a, labels_, umask)
                # loss_tv = cls_loss(lp_t_v, labels_, umask)
                # loss_va = cls_loss(lp_v_a, labels_, umask)
                # loss_vt = cls_loss(lp_v_t, labels_, umask)
                # loss_a = cls_loss(lp_a, labels_, umask)
                # loss_t = cls_loss(lp_t, labels_, umask)
                # loss_v = cls_loss(lp_v, labels_, umask)
        ...
        if dataset in ['CMUMOSI', 'CMUMOSEI']:
            if first_stage:
                ...
                loss_t = reg_loss(lp_t, labels_, umask)
                loss_v = reg_loss(lp_v, labels_, umask)
                # loss_at = reg_loss(lp_a_t, labels_, umask)
                # loss_av = reg_loss(lp_a_v, labels_, umask)
                # loss_ta = reg_loss(lp_t_a, labels_, umask)
                # loss_tv = reg_loss(lp_t_v, labels_, umask)
                # loss_va = reg_loss(lp_v_a, labels_, umask)
                # loss_vt = reg_loss(lp_v_t, labels_, umask)
    ...
    for ii in range(args.num_folder):
        ...
        for epoch in range(args.epochs):
            ...
            if first_stage:
                ...
                if (test_acc)/7>best_acc_all:
                    best_fscore=(test_acc)/7
